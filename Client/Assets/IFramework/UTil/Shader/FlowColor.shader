...
Shader "Custom/xuliezhen" {
    Properties {
        _MainTexture ("MainTexture", 2D) = "white" {}
_Hcount("_Hcount",float)=4
_Vcount("_Vcount",float)=4
_Speed("_Speed",float)=4
    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                VertexOutput o;
                o.uv0 = TRANSFORM_TEX(v.texcoord0,_MainTexture);
                o.pos = UnityObjectToClipPos( v.vertex );
o.pos.y+=0.04;
                return o;
            }
            float4 frag(VertexOutput i) : COLOR {
float time=floor(_Time.w*_Speed);
float row=floor(time/_Hcount);
float col=floor(time-row*_Hcount);
half2 uv=i.uv0+half2(col,row);
uv.x/=_Hcount;
uv.y/=_Vcount;
                float4 _MainTexture_var = tex2D(_MainTexture,TRANSFORM_TEX(uv, _MainTexture));
 if	(_MainTexture_var.r<0.01)discard;
